...
function AccountSettings({ LayoutComponent }) {
    ...
    const updateStoredUser = (newData) => {
        ...
        try {
            ...
            // Trigger a custom event for same-tab reactivity
                    ...
                    <div className="relative bg-gray-900 border border-white/10 rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl scale-in duration-300">
                        ...
                        <form onSubmit={handleChangePassword} className="space-y-5">
                            ...
                            <div className="space-y-2">
                                ...
                                {!passErrors.confirm && passForm.confirm && passForm.new === passForm.confirm && (
                                    <p className="text-green-500 text-[10px] font-bold ml-1 flex items-center gap-1">
            ...
            <div className="max-w-6xl mx-auto py-6 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
                <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="bg-gray-900/40 border border-white/5 rounded-[2.5rem] p-8 flex flex-col items-center text-center backdrop-blur-xl sticky top-8 shadow-2xl overflow-hidden">
                            ...
                            <div className="relative group mb-6 z-10">
                                <div className="w-32 h-32 rounded-[2rem] bg-gradient-to-tr from-gray-800 to-black border-4 border-gray-900 flex items-center justify-center text-5xl font-black text-white shadow-[0_20px_40px_rgba(0,0,0,0.4)] overflow-hidden transition-all group-hover:scale-105 duration-500">
                                    {(userData?.profile_picture || userData?.profile_pic) ? (
                                        <img
                                            src={
                                                (userData?.profile_picture || userData?.profile_pic).startsWith('http')
                                                    ? (userData?.profile_picture || userData?.profile_pic)
                                                    : `${API_BASE.endsWith('/') ? API_BASE.slice(0, -1) : API_BASE}${(userData?.profile_picture || userData?.profile_pic).startsWith('/') ? (userData?.profile_picture || userData?.profile_pic) : '/' + (userData?.profile_picture || userData?.profile_pic)}`
                                            }
                                            alt="Profile"
                    ...
                    <div className="lg:w-2/3 space-y-6">
                            ...
                            <div className="flex items-center justify-between mb-10">
                                ...
                                {!isEditing && (
                                    <div className="flex items-center gap-2 px-4 py-2 bg-gray-800/40 rounded-xl border border-white/5">
                                        ...
                                        <span className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Locked</span>
                                    </div>
                                )}
                            </div>
                        ...
                        {/* Employment Details */}
                        <div className="bg-gray-900/40 border border-white/5 rounded-[2.5rem] p-10 backdrop-blur-xl shadow-2xl relative overflow-hidden">
                            <div className="flex items-center gap-5 mb-10">
                                <div className="p-4 bg-gray-800 text-white rounded-[1.25rem] border border-white/5 shadow-xl">
                                    <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2-2H5a2 2 0 002-2v10a2 2 0 002 2z" />
                                    </svg>
                                </div>
                                <h3 className="text-2xl font-black text-white tracking-tight">Employment Status</h3>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="group p-8 bg-black/20 rounded-[2rem] border border-white/5 hover:border-red-600/30 transition-all">
                                    <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] mb-3">Member Since</p>
                                    <p className="text-gray-300 font-bold text-xl">{formatDate(userData?.created_at)}</p>
                                </div>
                                <div className="group p-8 bg-black/20 rounded-[2rem] border border-white/5 hover:border-red-600/30 transition-all">
                                    <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] mb-3">User Role</p>
                                    <p className="text-red-500 font-black text-xl">{userData?.role_display || userData?.role}</p>
                                </div>
                                <div className="group p-8 bg-gradient-to-br from-red-600/10 to-transparent rounded-[2rem] border border-red-600/20 shadow-xl shadow-red-600/5">
                                    <p className="text-[10px] font-black text-red-500/60 uppercase tracking-[0.2em] mb-3">Branch Assigned</p>
                                    <p className="text-white font-black text-xl truncate">{userData?.branch_name || "Headquarters"}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-900/40 border border-white/5 rounded-[2.5rem] p-10 backdrop-blur-xl shadow-2xl overflow-hidden group">
                            ...
                            <div className="bg-white/5 rounded-[2rem] p-10 flex flex-col md:flex-row items-center justify-between gap-8 border border-white/5 relative">
                                <div className="text-center md:text-left">
                                    ...
                                    <p className="text-gray-500 text-sm font-medium mt-2 max-w-sm">Protect your account with a strong and unique password.</p>
                                </div>
                                <button

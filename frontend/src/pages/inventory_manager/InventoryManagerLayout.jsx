...
const PAGE_TITLES = {
  "/inventory-manager/dashboard": "Dashboard",
  "/inventory-manager/inventory": "Inventory",
  "/inventory-manager/transactions": "Transaction History",
};

function InventoryManagerLayout({ children }) {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const location = useLocation();

  useEffect(() => {
    const title = PAGE_TITLES[location.pathname] ?? "Inventory Manager";
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="lg:ml-[var(--sidebar-width)] min-h-screen flex flex-col">
        <header className="lg:hidden flex items-center gap-4 px-4 py-3 bg-gray-900 sticky top-0 z-10 shadow-md border-b border-gray-800">
          ...
          <img src={logo} alt="Otokwikk" className="h-8 object-contain" />

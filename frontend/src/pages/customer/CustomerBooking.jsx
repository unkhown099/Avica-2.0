...
const statusConfig = {
  ...
  no_show:   { label: "No Show",    color: "bg-red-600/20 text-red-300 border-red-600/30" },
  done:      { label: "Completed",  color: "bg-blue-600/20 text-blue-400 border-blue-600/30" },
  rescheduled:{ label: "Reschedule Proposed", color: "bg-indigo-600/20 text-indigo-300 border-indigo-600/30" },
};
...
function SelectedBadge({ size = "md" }) {
  ...
  return (
    <div className={`${dim} rounded-full bg-red-600 border-2 border-red-400 flex items-center justify-center shadow-md shadow-red-600/40 flex-shrink-0`}>
  ...
  return (
    <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-white/8 flex-shrink-0">
      <div className="min-w-0 mr-3">
        ...
        {subtitle && (
          <p className="text-gray-500 text-[10px] sm:text-xs mt-0.5 truncate">{subtitle}</p>
        )}
      </div>
      ...
      <div className="flex items-center gap-1 flex-wrap">
        {pages.map((p) => {
          ...
          if (ellipsisBefore || ellipsisAfter)
            return (
              <span key={`d-${p}`} className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center text-gray-600 text-xs">…</span>
            );
          if (!show) return null;
          return (
            <button
        ...
        return (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center gap-1 flex-shrink-0">
              <div className={`w-5 h-5 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[9px] sm:text-xs font-black transition-all duration-300 ${done ? "bg-red-600 text-white shadow-md shadow-red-600/40" : active ? "bg-red-600/20 border-2 border-red-500 text-red-400" : "bg-white/5 border border-white/10 text-gray-600"}`}>
                {done ? (
              ...
              <span className={`text-[7px] sm:text-[10px] font-semibold tracking-wide uppercase whitespace-nowrap ${active ? "text-red-400" : done ? "text-gray-400" : "text-gray-600"}`}>
                {label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`h-px flex-1 mx-1 mb-3 sm:mb-4 transition-all duration-500 ${done ? "bg-red-600" : "bg-white/8"}`} />
            )}
          </React.Fragment>
        );
      })}
...
function CancelBookingModal({ booking, onClose, onConfirm }) {
  ...
  const cancelReasons = [
    "Change of plans","Found a better service provider","Schedule conflict",
    "Vehicle not available","Emergency","Other",
  ];
    ...
    <CenterModal onClose={onClose}>
      <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-white/8 flex-shrink-0">
        <div className="min-w-0 mr-3">
          ...
          <p className="text-gray-500 text-[10px] sm:text-xs mt-0.5 truncate max-w-[200px] sm:max-w-xs">{serviceName}</p>
        </div>
        <CloseBtn onClick={onClose} />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 space-y-4">
        <div className="bg-red-600/10 rounded-xl border border-red-600/20 px-3 py-2.5 sm:p-4 flex items-start gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-red-400 text-xs sm:text-sm font-semibold mb-0.5">This action cannot be undone</p>
        ...
        {reason === "Other" && (
          <div>
            <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">Please specify</p>
            <textarea
              rows={3}
        ...
        <div className="bg-white/4 rounded-xl p-3 sm:p-4 border border-white/8">
          ...
          <div className="space-y-1 text-xs sm:text-sm">
            {[
              ...
              booking.branch && ["Branch", typeof booking.branch === "object" ? booking.branch.name : booking.branch],
...
function DamageDetectionModal({ onClose, onBack }) {
  ...
  const handleImageUpload = (e) => {
    ...
    setImages((prev) => [...prev, ...files.map((file) => ({ file, preview: URL.createObjectURL(file), name: file.name }))]);
    setError("");
  };
  ...
  const analyzeDamage = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/analyze-damage/`, {
        method: "POST",
  ...
  return (
    <SlidePanel onClose={onClose}>
      <PanelHeader title="AI Damage Detection" accent="AI Damage" subtitle="Upload photos for AI analysis" onClose={onClose} />
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-5">
        {!analysisResult ? (
          <>
            <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Upload Vehicle Photos</p>
            <div
            ...
            >
              ...
              <p className="text-gray-600 text-[10px] sm:text-xs">JPG, PNG, HEIC — max 10MB each</p>
            </div>
            {images.length > 0 && (
              <div className="mb-4 sm:mb-6">
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Uploaded ({images.length})</p>
                <div className="grid grid-cols-2 gap-2">
                  {images.map((img, i) => (
                    <div key={i} className="relative group">
                      <img src={img.preview} alt={`Damage ${i + 1}`} className="w-full h-20 sm:h-32 object-cover rounded-xl border border-white/10" />
                      <button
            ...
            <div className="bg-white/4 rounded-xl p-3 sm:p-4 border border-white/8">
              <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Tips for best results</p>
              <ul className="space-y-1.5 text-gray-400">
                {["Take photos in good lighting","Capture damages from multiple angles","Include reference object for scale","Avoid blurry or dark images"].map((t) => (
          ...
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-4">
              ...
              <div>
                <p className="text-white font-bold text-sm sm:text-lg">Analysis Complete</p>
            ...
            <div className="space-y-2 mb-4">
              ...
              {analysisResult.damages.map((d, i) => (
                <div key={i} className="bg-white/4 rounded-xl p-3 border border-white/8">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-white font-semibold text-xs sm:text-sm">{d.type}</span>
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${d.severity === "Minor" ? "bg-yellow-600/20 text-yellow-400" : d.severity === "Moderate" ? "bg-orange-600/20 text-orange-400" : "bg-red-600/20 text-red-400"}`}>
                      {d.severity}
                    </span>
                  </div>
                  <p className="text-gray-400 text-[10px] sm:text-xs">Location: {d.location}</p>
                  <p className="text-gray-500 text-[9px] sm:text-[10px]">Confidence: {(d.confidence * 100).toFixed(0)}%</p>
                </div>
              ))}
            </div>
            <div className="mb-4">
              <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Recommendations</p>
            ...
            <div className="bg-red-600/10 rounded-xl p-3 sm:p-4 border border-red-600/20">
              ...
              <p className="text-gray-500 text-[9px] sm:text-[10px] mt-1">*Final cost may vary after inspection</p>
            </div>
          </div>
        )}
        {error && (
...
function NewBookingModal({ onClose, onSuccess, initialDamageData, initialServiceId = null }) {
  ...
  const [form, setForm] = useState({
    ...
    vehicle: "", plateNumber: "", notes: initialDamageData
      ...
      : "",
    damageData: initialDamageData, preferredEmployee: null,
  });
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/bookings/`, { headers: authHeaders() })
      ...
      .then((data) => { setUserBookings(Array.isArray(data) ? data : (data.results ?? [])); setUserBookingsLoaded(true); })
      .catch(() => setUserBookingsLoaded(true));
  }, []);

  useEffect(() => {
    if (!userBookingsLoaded) { setHasActiveBooking(false); return; }
    const activeExists = userBookings.some((b) => b.status === "pending" || b.status === "confirmed");
    setHasActiveBooking(activeExists);
    if (activeExists) setError("You already have an active booking. Please complete or cancel it before creating a new one.");
    else setError((prev) => (prev.includes("already have an active booking") ? "" : prev));
  }, [userBookings, userBookingsLoaded]);

  useEffect(() => {
    if (!form.date || !form.branch) { setAvailableSlots(null); setAvailabilityMeta(null); return; }
    setForm((prev) => ({ ...prev, time: "" }));
    setAvailableSlots(null); setAvailabilityMeta(null); setCheckingAvailability(true);
    const params = new URLSearchParams({ branch_id: String(form.branch.id), date: form.date });
    if (bookingMode === "specific" && form.preferredEmployee?.id)
      params.set("preferred_employee_id", String(form.preferredEmployee.id));
    fetch(`${API_BASE}/api/bookings/available-slots/?${params.toString()}`, { headers: authHeaders() })
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((data) => { setAvailableSlots(data.available_slots ?? {}); setAvailabilityMeta(data.meta ?? null); })
      .catch(() => { const f = {}; TIME_SLOTS.forEach((s) => { f[s] = true; }); setAvailableSlots(f); })
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL}/services/`, { headers: authHeaders() })
      ...
      .then((data) => setServices((Array.isArray(data) ? data : (data.results ?? [])).filter((s) => s.is_active !== false && (s.branches?.length ?? 0) > 0)))
  ...
  useEffect(() => {
    if (!form.branch?.id) { setEmployees([]); setEmployeesError(""); setForm((prev) => ({ ...prev, preferredEmployee: null })); return; }
    setEmployeesLoading(true); setEmployeesError("");
    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/queue/employees/?branch_id=${form.branch.id}`, { headers: authHeaders() })
      .then((r) => { if (!r.ok) throw new Error("Failed to load employees."); return r.json(); })
      .then((data) => {
      ...
      .catch((err) => { setEmployees([]); setEmployeesError(err.message || "Failed to load employees."); })
  ...
  const visibleTimeSlots = useMemo(() => {
    ...
    return dynamic.sort((a, b) => formatTimeForAPI(a).localeCompare(formatTimeForAPI(b)));
  }, [availableSlots]);

  const categoryOptions = useMemo(() => {
    const fromApi = categories.map((c) => (typeof c === "string" ? c : c?.name)).filter(Boolean);
  ...
  const filteredServices = useMemo(() => {
    ...
    return services.filter((s) => String(s.category || "").toLowerCase() === String(selectedCategory).toLowerCase());
  }, [services, selectedCategory]);

  useEffect(() => {
    if (selectedCategory === "all") return;
    const exists = categoryOptions.some((name) => String(name).toLowerCase() === String(selectedCategory).toLowerCase());
    if (!exists) setSelectedCategory("all");
  }, [categoryOptions, selectedCategory]);

  const scheduleWindowText = useMemo(() => {
    if (!availabilityMeta || !form.date) return "";
    if (availabilityMeta.closed) return availabilityMeta.closure_reason || "This branch is closed on the selected date.";
    ...
    if (availabilityMeta.break_start && availabilityMeta.break_end) text += ` (break ${availabilityMeta.break_start} - ${availabilityMeta.break_end})`;
    if (availabilityMeta.slot_duration) text += ` · ${availabilityMeta.slot_duration}-min slots`;
    return text;
  }, [availabilityMeta, form.date]);

  const canAdvance = () => {
    setError("");
    if (step === 0) { if (!form.service) { setError("Please select a service."); return false; } return true; }
    if (step === 1) { if (!form.branch) { setError("Please select a branch."); return false; } return true; }
    if (step === 2) { if (bookingMode === "specific" && !form.preferredEmployee?.id) { setError("Please select a specific employee."); return false; } return true; }
    if (step === 3) {
      if (!form.date) { setError("Please select a date."); return false; }
      if (form.date < todayISO()) { setError("Past dates are not allowed. Please select today or a later date."); return false; }
      if (hasActiveBooking) { setError("You already have an active booking."); return false; }
      if (!form.time) { setError("Please select a time slot."); return false; }
      if (!isSlotAvailable(form.time)) { setError("The selected time slot is no longer available."); setForm((p) => ({ ...p, time: "" })); return false; }
      return true;
    }
    return true;
  };

  const handleNext = () => { if (!canAdvance()) return; setStep((s) => s + 1); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (form.date < todayISO()) throw new Error("Past dates are not allowed. Please select today or a later date.");
      if (hasActiveBooking) throw new Error("You already have an active booking. Please complete or cancel it before creating a new one.");
      if (!isSlotAvailable(form.time)) throw new Error("This time slot is no longer available. Please go back and select another time.");
      ...
      if (!/^[a-zA-Z0-9]{1,8}$/.test(plateNumber)) throw new Error("Plate number must be alphanumeric and max 8 characters.");
      if (bookingMode === "specific" && !form.preferredEmployee?.id) throw new Error("Please select a specific employee.");
      ...
      const res = await fetch(`${API_BASE}/api/bookings/`, { method: "POST", headers: authHeaders(), body: JSON.stringify(payload) });
      if (!res.ok) {
        ...
        if (errorData && typeof errorData === "object") {
          ...
          if (Object.keys(nfe).length > 0) { setFieldErrors(nfe); throw new Error(Object.values(nfe)[0] || "Please check the form."); }
  ...
  const availableCount = availableSlots ? Object.values(availableSlots).filter((v) => v === true).length : 0;
  ...
  return (
    <SlidePanel onClose={onClose}>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-5">
        ...
        {step === 0 && (
          <div>
            <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Choose a Service</p>
            {!servicesLoading && !servicesError && (
              <div className="mb-3">
                    ...
                    {categoryOptions.map((cat) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (form.service && String(form.service.category || "").toLowerCase() !== String(cat).toLowerCase())
                            set("service", null);
                        }}
                ...
                {categoriesError && <p className="mt-1.5 text-[10px] text-yellow-500">{categoriesError}</p>}
              </div>
            )}
            {servicesLoading ? (
              ...
              <p className="text-center py-12 text-red-400 text-xs">{servicesError}</p>
            ) : filteredServices.length === 0 ? (
              <p className="text-center py-12 text-gray-500 text-xs">No services available.</p>
            ) : (
              <>
                {/* Mobile compact list */}
                <div className="flex flex-col gap-2 sm:hidden">
                  {filteredServices.map((s) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-lg shrink-0">{CATEGORY_ICON[s.category] ?? "🔧"}</div>
                        <div className="flex-1 min-w-0">
                          <div className={`font-bold text-xs truncate ${active ? "text-white" : "text-gray-300"}`}>{s.name}</div>
                          {s.category && <div className="text-gray-500 text-[9px]">{s.category}</div>}
                        </div>
                        <div className="text-right shrink-0">
                          <div className="text-red-400 font-black text-xs">₱{parseFloat(s.price || 0).toLocaleString()}</div>
                          {s.duration && <div className="text-gray-600 text-[9px]">⏱ {s.duration}</div>}
                        </div>
                ...
                <div className="hidden sm:grid sm:grid-cols-2 gap-3">
                  {filteredServices.map((s) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-2xl mb-2">{CATEGORY_ICON[s.category] ?? "🔧"}</div>
                        <div className={`font-bold text-sm mb-1 ${active ? "text-white" : "text-gray-300"}`}>{s.name}</div>
                        <div className="text-red-400 font-black text-base">₱{parseFloat(s.price || 0).toLocaleString()}</div>
                        {s.duration && <div className="text-gray-600 text-[10px] mt-1">⏱ {s.duration}</div>}
                        {s.category && <div className="text-gray-600 text-[10px]">{s.category}</div>}
                        {active && <div className="absolute top-3 right-3"><SelectedBadge /></div>}
                      </button>
                    );
                  })}
                </div>
        ...
        {step === 1 && (
          <div>
            <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Choose a Branch</p>
            {branchLoading ? (
              <div className="flex items-center justify-center py-12 text-gray-500 text-xs">
                <svg className="w-4 h-4 animate-spin mr-2" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              ...
              <p className="text-center py-12 text-red-400 text-xs">{branchError}</p>
            ) : (() => {
              const availableBranches = form.service
                ? branches.filter((b) => form.service.branches?.some((sb) => sb.id === b.id))
                : branches;
              if (availableBranches.length === 0)
                return <p className="text-center py-12 text-gray-500 text-xs">No branches available for the selected service.</p>;
              return (
                  ...
                  {availableBranches.map((b) => {
                    ...
                    return (
                      <button
                        key={b.id}
                        type="button"
                        onClick={() => { set("branch", b); if (bookingMode === "specific") set("preferredEmployee", null); }}
                      ...
                      >
                        <div className={`w-7 h-7 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center shrink-0 mt-0.5 transition-all duration-200 ${active ? "bg-red-600 shadow-md shadow-red-600/40" : "bg-white/8"}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-gray-500 text-[9px] sm:text-[10px] truncate">{b.address}</div>
                          <div className="flex items-center gap-2 flex-wrap mt-1">
                            <span className="text-gray-600 text-[9px] sm:text-[10px]">{b.hours}</span>
                            <span className="text-green-400 text-[9px] sm:text-[10px] font-semibold">{b.slots} slots open</span>
                          </div>
                        </div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                { key: "general",  label: "General Booking",       desc: "Any available employee will be assigned.", accent: "emerald" },
                { key: "specific", label: "Book Specific Employee", desc: "Choose the employee you want.",           accent: "red" },
              ].map(({ key, label, desc }) => {
                ...
                return (
                  <button
                    key={key}
                    type="button"
                    onClick={() => { setBookingMode(key); if (key === "general") set("preferredEmployee", null); }}
                  ...
                  >
                    <div className="text-white font-bold text-sm sm:text-base pr-6">{label}</div>
                    <div className="text-gray-400 text-[10px] sm:text-xs mt-1">{desc}</div>
                    {active && <div className="absolute top-3 right-3"><SelectedBadge size="sm" /></div>}
                  </button>
                );
              })}
            </div>
            {bookingMode === "specific" && (
              <div>
                <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Select Employee</p>
                {employeesLoading ? (
                  <p className="text-gray-500 text-xs py-6">Loading employees...</p>
                ) : employeesError ? (
                  <p className="text-red-400 text-xs py-6">{employeesError}</p>
                ) : employees.length === 0 ? (
                  <p className="text-gray-500 text-xs py-6">No active employees for this branch.</p>
                ) : (
                  <div className="space-y-2">
                    {employees.map((emp) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`font-semibold text-xs sm:text-sm pr-6 ${active ? "text-white" : "text-gray-200"}`}>{emp.full_name}</div>
                          <div className="text-gray-500 text-[10px]">Branch: {emp.branch || form.branch?.name || "Unassigned"}</div>
                          {active && <div className="absolute top-1/2 right-3 -translate-y-1/2"><SelectedBadge size="sm" /></div>}
                        </button>
                      );
                    })}
                  </div>
          ...
          <div className="space-y-4 sm:space-y-5">
            <div>
              <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 sm:mb-3">
                Pick a Date <span className="text-yellow-500 text-[8px] sm:text-[10px]">(Tomorrow onward)</span>
              </p>
              <input
                type="date"
                min={tomorrowISO()}
              ...
              {form.date && form.date < tomorrowISO() && (
                <p className="text-yellow-500 text-[8px] sm:text-[10px] mt-1">Past dates are not allowed. Please select a future date.</p>
              )}
              {hasActiveBooking && (
                <p className="text-red-400 text-[8px] sm:text-[10px] mt-1">You already have an active booking. Please complete or cancel it before creating a new one.</p>
              )}
              {scheduleWindowText && (
                <p className="text-gray-500 text-[8px] sm:text-[10px] mt-1">{scheduleWindowText}</p>
              )}
            </div>
            ...
            <div>
              <p className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">
                Pick a Time Slot
                {checkingAvailability && <span className="ml-2 text-gray-500 font-normal normal-case text-[9px]">(Checking…)</span>}
              </p>
              {form.date && availableSlots === null && !checkingAvailability && (
                <p className="text-gray-500 text-[10px] sm:text-xs mb-2">Select a date and branch to see slots.</p>
              )}
              {availableSlots !== null && visibleTimeSlots.length === 0 && (
                <p className="text-gray-500 text-[10px] sm:text-xs mb-2">No available slots for this date.</p>
              )}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
                {visibleTimeSlots.map((t) => {
                  ...
                  const isDisabled = !slotsLoaded || !slotAvailable || !dateValid || hasActiveBooking || checkingAvailability;
                  return (
                    <button
                      key={t}
                      type="button"
                      ...
                      disabled={isDisabled}
                      title={
                        !dateValid ? "Select a valid date first"
                          : hasActiveBooking ? "You already have an active booking"
                          : !slotsLoaded || checkingAvailability ? "Loading availability…"
                          : !slotAvailable ? "This slot is fully booked"
                          : ""
                      }
            ...
            {form.date && form.time && (
              <div className="flex items-center gap-2 bg-white/4 rounded-xl px-3 py-2.5 border border-white/8">
                <svg className="w-3 h-3 sm:w-4 sm:h-4 text-red-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-4">
            {form.damageData && (
              <div className="bg-blue-600/10 border border-blue-600/20 rounded-xl p-3">
                ...
                <p className="text-gray-400 text-[9px] sm:text-[10px]">AI analysis results will be attached to your booking</p>
              </div>
            )}

            {[
              { label: "Vehicle Type", key: "vehicle", placeholder: "e.g. Toyota Vios, Honda Civic...", apiKey: "vehicle" },
              { label: "Plate Number", key: "plateNumber", placeholder: "e.g. ABC1234 (max 8 chars)", apiKey: "plate_number" },
              ...
              <div key={key}>
                ...
                {key === "plateNumber" && (
                  <div className="flex items-center justify-between mt-1">
                    <p className={`text-[9px] sm:text-[10px] transition-colors ${form.plateNumber.length === 8 ? "text-yellow-500" : "text-gray-600"}`}>
                      {form.plateNumber.length}/8
                    </p>
                    {fieldErrors[apiKey] && (
                      <p className="text-red-400 text-[9px] sm:text-[10px]">{fieldErrors[apiKey]}</p>
                    )}
                  </div>
                )}
                {key !== "plateNumber" && fieldErrors[apiKey] && (
                  <p className="text-red-400 text-[9px] sm:text-[10px] mt-1">{fieldErrors[apiKey]}</p>
                )}
              </div>
            ))}

            <div>
              <label className="block text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">Employee Assignment</label>
              <div className="w-full bg-white/5 border border-white/10 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-white text-xs sm:text-sm">
                {bookingMode === "specific"
                  ? form.preferredEmployee?.full_name || "Specific employee selected"
            ...
            <div className="rounded-2xl border border-white/10 bg-white/3 overflow-hidden">
              ...
              <div className="divide-y divide-white/5">
                {[
                  ["Booking Type", bookingMode === "specific" ? "Specific Employee" : "General Booking"],
                  ["Service", form.service?.name],
                  ["Branch", form.branch?.name],
                  ["Date", form.date],
                  ["Time", form.time],
                  ["Employee", form.preferredEmployee?.full_name || "No preference"],
                  ["Price", form.service ? `₱${parseFloat(form.service.price || 0).toLocaleString()}` : "—", true],
                ].map(([label, value, highlight]) => (
                  <div key={label} className="flex items-center justify-between px-3 sm:px-4 py-2">
                    <span className="text-gray-500 text-[10px] sm:text-xs">{label}</span>
                    <span className={`text-[10px] sm:text-xs font-semibold ${highlight ? "text-red-400 text-sm font-black" : "text-white"}`}>
                      {value || "—"}
                    </span>
                  </div>
                ))}
...
function BookingsPage() {
  ...
  useEffect(() => {
    ...
    setShowOptionModal(false); setShowDamageModal(false); setDamageData(null);
  ...
  const handleBookingSuccess = (newBooking) => {
    ...
    setShowBookingModal(false); setShowDamageModal(false);
    setDamageData(null); setPrefillServiceId(null); setPage(1);
    showToast("Your booking was submitted successfully!");
  };

  const handleOptionSelect = (option) => {
    setShowOptionModal(false);
    if (option === "booking") { setPrefillServiceId(null); setShowBookingModal(true); return; }
    setShowDamageModal(true);
  };

  const handleDamageComplete = (data) => {
    if (data.type === "booking" && data.damageData) {
      setDamageData(data.damageData); setShowDamageModal(false); setShowBookingModal(true);
    }
  };
  ...
  return (
    <CustomerLayout>
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-red-950/30 p-3 sm:p-5 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 sm:mb-8">
          ...
          <div className="relative group self-start sm:self-auto">
            <button
              onClick={() => { if (!newBookingDisabled) setShowOptionModal(true); }}
            ...
            {newBookingDisabled && (
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 z-10 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <div className="bg-gray-900 border border-red-600/30 rounded-xl px-3 py-2 text-center shadow-xl">
                  <p className="text-red-400 text-[10px] font-semibold leading-tight">Active booking in progress</p>
        ...
        {!loading && hasActiveBooking && (
          <div className="mb-5 sm:mb-6 flex items-center gap-3 bg-yellow-600/10 border border-yellow-600/25 rounded-2xl px-4 py-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-[10px] sm:text-xs">New bookings are blocked until your current one is completed or cancelled.</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 sm:gap-4 mb-6 sm:mb-8">
          {[
            ...
            <div key={label} className="bg-gradient-to-br from-gray-900 to-red-950/10 rounded-xl p-3 sm:p-5 border border-white/5 text-center hover:border-white/10 transition-colors duration-200">
        ...
        <div className="space-y-2.5 sm:space-y-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:flex-wrap gap-x-4 sm:gap-x-5 gap-y-1.5 sm:gap-y-2 text-gray-400 text-[10px] sm:text-xs">
                      {[
                        ...
                        booking.branch && { icon: "M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0zM15 11a3 3 0 11-6 0 3 3 0 016 0z", text: typeof booking.branch === "object" ? booking.branch.name : booking.branch },
          ...
          {!loading && !fetchError && filtered.length === 0 && (
            <div className="text-center py-16 sm:py-24 text-gray-500">
              <svg className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <p className="text-sm sm:text-lg font-semibold">No bookings found</p>
            </div>
          )}
        </div>
      ...
      {/* Modals */}
      {showOptionModal && (
        <OptionSelectorModal onClose={() => setShowOptionModal(false)} onSelectOption={handleOptionSelect} />
      )}
      {showBookingModal && (
        <NewBookingModal
          onClose={() => { setShowBookingModal(false); setDamageData(null); setPrefillServiceId(null); }}

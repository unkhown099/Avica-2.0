...
function StatCard({ title, value, icon, accentBg, accentText, border, sub }) {
  return (
    <div className={`bg-gray-900/60 border ${border} rounded-2xl p-4 sm:p-5 backdrop-blur-sm hover:border-opacity-60 transition-all`}>
      <div className="mb-3 sm:mb-4">
        <div className={`${accentBg} ${accentText} p-2 sm:p-3 rounded-xl w-fit`}>{icon}</div>
      </div>
      <div className="text-xl sm:text-2xl font-black text-white mb-1 truncate">{value ?? "—"}</div>
...
function UsersTable({ users, loading, onEdit, onDelete, onStatusToggle }) {
  ...
  const getRoleBadge = (role) => {
    const colors = {
      ...
      inventory_manager: 'bg-orange-500/10 text-orange-400 border-orange-500/20',
              ...
              <tr key={user.id} className="hover:bg-white/[0.02] transition-colors">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {user.profile?.phone && (
                        <div className="text-gray-600 text-xs mt-1">{user.profile.phone}</div>
                      )}
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium border ${getRoleBadge(user.profile?.role)}`}>
                    {roleLabels[user.profile?.role] || user.account_type}
                  </span>
                  {user.profile?.branch && (
                    <div className="text-gray-600 text-xs mt-1">{user.profile.branch}</div>
                  )}
                  {user.profile?.loyalty_points && (
                    <div className="text-green-500 text-xs mt-1">{user.profile.loyalty_points} pts</div>
                  )}
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-col gap-1">
                    ...
                    {!user.email_verified && user.account_type === 'customer' && (
                      <span className="text-yellow-500 text-xs">Email not verified</span>
                    )}
                  </div>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={user.is_active 
      ...
      <div className="md:hidden divide-y divide-white/5">
        {users.map((user) => (
          <div key={user.id} className="p-4 hover:bg-white/[0.02] transition-colors">
            ...
            {!user.email_verified && user.account_type === 'customer' && (
              <div className="mt-2 text-yellow-500 text-xs">Email not verified</div>
            )}
          </div>
        ))}
      </div>
...
function AddUserForm({ onSubmit, onCancel, branches, roles, loading }) {
  ...
  const filteredRoles = formData.account_type === 'staff' 
  ...
  return (
    <div className="bg-gray-900/60 border border-white/5 rounded-2xl p-4 sm:p-6 backdrop-blur-sm">
      <h3 className="text-lg sm:text-xl font-bold text-white mb-4">Add New User</h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, account_type: e.target.value, role: '' })}
              className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-red-500"
            >
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="sendEmail" className="text-xs sm:text-sm text-gray-400">
...
function EditUserModal({ user, onClose, onSave, branches, loading }) {
  ...
  const staffRoles = ['super_admin', 'admin', 'branch_manager', 'employee', 'inventory_manager'];
            ...
            <>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, staff_branch: e.target.value })}
                  className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-red-500"
                >
                  <option value="">Select branch</option>
                  {branches.map(branch => (
...
function RolesPermissionsModal({ roles, onClose }) {
  ...
  const [roleSettings, setRoleSettings] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("superadmin_role_settings") || "null");
      return roles.map((role) => ({
        key: role,
        label: saved?.find((item) => item.key === role)?.label || roleLabels[role] || role,
        permissions: saved?.find((item) => item.key === role)?.permissions || defaultPermissions,
      }));
    } catch {
      return roles.map((role) => ({
        key: role,
    ...
    setRoleSettings((prev) => {
      ...
      return roles.map((role) => {
        ...
        return {
          ...
          permissions: savedItem?.permissions || existing.permissions || defaultPermissions,
        };
      });
    });
  }, [roles]);
  ...
  const handleLabelChange = (key, value) => {
    setRoleSettings((prev) => prev.map((item) => item.key === key ? { ...item, label: value } : item));
  };

  const togglePermission = (key, perm) => {
    setRoleSettings((prev) => prev.map((item) =>
      item.key === key
        ? { ...item, permissions: { ...item.permissions, [perm]: !item.permissions[perm] } }
        : item
    ));
  };

  const saveSettings = () => {
    localStorage.setItem("superadmin_role_settings", JSON.stringify(roleSettings));
      ...
      <div className="bg-gray-900 border border-white/10 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-gray-900 border-b border-white/10 px-4 sm:px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 text-sm">Super admins can update role labels and permission defaults here.</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-white p-1">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
        ...
        <div className="p-4 sm:p-6 space-y-4">
          <div className="rounded-2xl border border-white/10 bg-gray-800/80 p-4 text-sm text-gray-300">
            Only users with Super Admin access can edit these role settings. The interface is intentionally lightweight so it can be expanded later with real role permission definitions.
          </div>

          {roleSettings.length > 0 ? (
            <div className="space-y-4">
              {roleSettings.map((role) => (
                <div key={role.key} className="rounded-2xl border border-white/10 bg-gray-900/80 p-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-white">{roleLabels[role.key] || role.key}</div>
                      <div className="text-xs text-gray-500">Role key: {role.key}</div>
                    </div>
                    <div className="w-full sm:w-72">
                      ...
                      <input
                        type="text"
                        value={role.label}
                        onChange={(e) => handleLabelChange(role.key, e.target.value)}
                        className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-red-500"
                      />
                    </div>
                  </div>
...
export default function SuperAdminUsers() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [usersRes, branchesRes, rolesRes] = await Promise.all([
        fetch(`${API_BASE}/super-admin/users/`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${API_BASE}/branches/`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${API_BASE}/super-admin/roles/`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleDeleteUser = async (user) => {
    ...
    try {
      const token = localStorage.getItem("access_token") ?? sessionStorage.getItem("access_token");
      const response = await fetch(`${API_BASE}/super-admin/users/${user.id}/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleAddUser = async (formData) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/super-admin/create/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-red-950/30 -m-4 sm:-m-8 p-3 sm:p-8">
        ...
        {error && (
          <div className="mb-6 flex items-center gap-3 bg-red-500/10 border border-red-500/30 rounded-xl px-4 sm:px-5 py-3 sm:py-4">
            ...
            <button onClick={fetchData} className="text-red-400 hover:text-red-300 text-sm font-semibold underline">
              Retry
            </button>
          </div>
        )}

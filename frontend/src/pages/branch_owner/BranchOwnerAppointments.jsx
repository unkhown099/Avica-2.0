        ...
        {Array.from({ length: totalDays }, (_, i) => i + 1).map((day) => {
          ...
          return (
            <button
              ...
              className={`aspect-square flex flex-col items-center justify-center rounded-xl text-xs sm:text-sm font-semibold transition-all ${isSelected ? "bg-red-600 text-white shadow-lg shadow-red-600/30" : "hover:bg-white/5 text-gray-400 hover:text-white"}`}
            >
              <span>{day}</span>
              {dots.length > 0 && (
            ...
            {loading ? (
              <div className="space-y-4">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="bg-gray-800/60 rounded-xl p-4 sm:p-5 animate-pulse"
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gray-700" />
                    ...
                    >
                      <div className="flex items-start justify-between mb-3 sm:mb-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-sm font-black shrink-0 ${apt.status === "confirmed" ? "bg-emerald-500/20 text-emerald-400" : "bg-amber-500/20 text-amber-400"}`}
                          >
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold border capitalize shrink-0 ml-2 ${STATUS_STYLE[apt.status] ?? STATUS_STYLE.pending}`}
                        >
                          {apt.status}
                        </span>
                      </div>

  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3 sm:mb-4">
        <div className={`${accentBg} ${accentText} p-2.5 sm:p-3 rounded-xl`}>
          {icon}
        </div>
      </div>
  ...
  return (
    <div className="bg-gray-900/60 border border-white/5 rounded-2xl p-4 sm:p-5 animate-pulse">
      <div className="flex justify-between mb-4">
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gray-800" />
      </div>
      <div className="h-6 sm:h-7 w-24 bg-gray-800 rounded mb-2" />
...
export default function BranchOwnerDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        fetchFirstAvailable(
          ["/owner/services/?performance=1", "/owner/services/"],
          [],
        ),
      ]);
      ...
      const analyticsRevenueByBranch = toArray(
        dashboardAnalytics?.revenue_by_branch,
      ).map((row) => ({
      ...
      const fallbackTopServices = toArray(ownerServicesData).map((service) => ({
        ...
        count: Number(
          service.count ?? service.bookings ?? service.total_bookings ?? 0,
        ),
  ...
  const totalBranchRev = branchRevenue.reduce(
  ...
  const servicesByRevenue = useMemo(
    () =>
      [...services]
        .map((s) => ({
          ...s,
          _serviceName: s.service ?? s.name ?? s.service_name ?? "—",
          _count: Number(s.count ?? 0),
          _revenue: Number(s.revenue ?? 0),
          _avgTime: s.avg_time ?? "—",
        }))
  ...
  const handlePrintDashboard = async () => {
    ...
    try {
      ...
      printWindow.document.write(
        `<!doctype html><html><head><title>Owner Dashboard — ${viewLabel}</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{background:#fff;}img{width:100%;height:auto;display:block;}@page{size:A4 portrait;margin:8mm;}</style></head><body><img src="${imgData}" /><script>window.onload=function(){setTimeout(function(){window.print();window.close();},500);};<\/script></body></html>`,
      );
  ...
  return (
    <BranchOwnerLayout title="" subtitle="">
      <div
        ...
        className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-red-950/30 -m-4 sm:-m-8 p-4 sm:p-8"
      >
        {/* Header */}
        <div className="mb-6">
          ...
          <span className="hidden sm:inline">Export CSV</span>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
              <div className="xl:col-span-2 bg-gray-900/60 border border-white/5 rounded-2xl p-4 sm:p-6 backdrop-blur-sm">
                <div className="mb-4 sm:mb-6">
                  ...
                  <p className="text-gray-500 text-sm mt-0.5">
                    Owner dashboard trend
                  </p>
                </div>
                {loading ? (
                  <div className="h-56 sm:h-72 bg-gray-800/40 rounded-xl animate-pulse" />
                ) : trend.length === 0 ? (
                  <div className="h-56 sm:h-72 flex items-center justify-center text-gray-600 text-sm">
                    No data yet
                  </div>
                ) : (
              ...
              <div className="bg-gray-900/60 border border-white/5 rounded-2xl p-4 sm:p-6 backdrop-blur-sm">
                ...
                {loading ? (
                  <div className="h-36 sm:h-44 bg-gray-800/40 rounded-xl animate-pulse mb-6" />
                ) : branchRevenue.length === 0 ? (
                  <div className="h-36 sm:h-44 flex items-center justify-center text-gray-600 text-sm">
                    No data yet
                  </div>
                ) : (
                  <div className="h-36 sm:h-44 flex items-center justify-center mb-6">
                    <Doughnut
                  ...
                  {branchRevenue.map((b, i) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-bold text-white">
                        {fmtCompactMoney(b.revenue)}
                      </span>
                      {totalBranchRev > 0 && (
                        <span className="text-xs text-gray-600">
                          {Math.round(
                            (Number(b.revenue ?? 0) / totalBranchRev) * 100,
                          )}
                          %
                        </span>
                      )}
      ...
      <span className="hidden sm:inline">Export CSV</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
              ...
              <StatCard
                ...
                border="border-blue-500/20"
                sub={
                  sortedBranchRevenue[0]
                    ? fmtCompactMoney(sortedBranchRevenue[0].revenue)
                    : undefined
                }
                icon={
                  <svg
                ...
                <Bar
                  data={{
                    ...
                    datasets: [
                      {
                        label: "Revenue",
                        data: sortedBranchRevenue.map((r) =>
                          Number(r.revenue ?? 0),
                        ),
      ...
      <span className="hidden sm:inline">Export CSV</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
              ...
              <StatCard
                ...
                sub={
                  topCustomer
                    ? fmtMoney(topCustomer.total_spent ?? 0)
                    : undefined
                }
              ...
              <StatCard
                ...
                value={
                  customers.length
                    ? `${(customers.reduce((sum, c) => sum + Number(c.avg_rating ?? 0), 0) / (customers.filter((c) => c.avg_rating != null).length || 1)).toFixed(1)} / 5`
                    : "—"
                }
                accentBg="bg-blue-500/10"
                accentText="text-blue-400"
            ...
            <div className="md:hidden space-y-3 mb-4">
              {customersPagination.paginatedItems.length === 0 ? (
                ...
                customersPagination.paginatedItems.map((c, i) => (
                  <div
                    key={c.id ?? i}
                    className="bg-gray-900/60 border border-white/5 rounded-2xl p-4"
                  >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
              ...
              <StatCard
                ...
                value={(
                  ...
                  appointmentStatusCounts.noShow
                ).toLocaleString()}
                accentBg="bg-red-500/10"
                accentText="text-red-400"
                border="border-red-500/20"
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${STATUS_STYLE[statusKey] ?? "bg-gray-500/20 text-gray-400 border-gray-500/30"}`}
                        >
                          {STATUS_LABEL[statusKey] ?? apt.status}
                        </span>
                      </div>
                      <div className="text-gray-400 text-xs">
                        {apt.service ?? "—"} · {apt.branch_name ?? "—"}
                      </div>
                    </div>
                  );
                })
    ...
    >
      ...
      <span className="hidden sm:inline">Export CSV</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
              ...
              <StatCard
                title="Low Stock"
                value={inventoryCounts.low.toLocaleString()}
                accentBg="bg-amber-500/10"
                accentText="text-amber-400"
                border="border-amber-500/20"
                icon={
              ...
              <StatCard
                title="Out of Stock"
                value={inventoryCounts.out.toLocaleString()}
                accentBg="bg-red-500/10"
                accentText="text-red-400"
                border="border-red-500/20"
                icon={
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${INV_STYLE[key]}`}
                      >
                        {key === "ok"
                          ? "In Stock"
                          : key === "low"
              ...
              {inventoryPagination.paginatedItems.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="col-span-2">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold border ${INV_STYLE[key]}`}
                      >
                        {key === "ok"
                          ? "In Stock"
                          : key === "low"
      ...
      <span className="hidden sm:inline">Export CSV</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
              <StatCard
                ...
                value={servicesByRevenue
                  .reduce((sum, s) => sum + Number(s._count ?? 0), 0)
                  .toLocaleString()}
                accentBg="bg-blue-500/10"
                accentText="text-blue-400"
                border="border-blue-500/20"
              ...
              <StatCard
                title="Top Service"
                value={servicesByRevenue[0]?._serviceName ?? "—"}
                accentBg="bg-red-500/10"
                accentText="text-red-400"
                border="border-red-500/20"
                ...
                icon={
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M5 3v4M3 5h4m5-2l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"
                    />
                  </svg>
                }
              />
              <StatCard
                ...
                value={fmtMoney(
                  servicesByRevenue.reduce(
                    (sum, s) => sum + Number(s._revenue ?? 0),
                    0,
                  ),
                )}
                accentBg="bg-emerald-500/10"
            ...
            <div className="md:hidden space-y-3 mb-4">
              {servicesPagination.paginatedItems.length === 0 ? (
                ...
                servicesPagination.paginatedItems.map((s, i) => {
                  const totalRev = servicesByRevenue.reduce(
                    (acc, row) => acc + Number(row._revenue ?? 0),
                    0,
                  );
                  const pct = totalRev
                    ? ((Number(s._revenue ?? 0) / totalRev) * 100).toFixed(1)
                    : "0.0";
                  return (
                    <div
                      key={s.id ?? i}
                    ...
                    >
                      ...
                      <div className="text-gray-500 text-xs">
                        {Number(s._count ?? 0)} services · {s._avgTime} · {pct}%
                        share
                      </div>
                    </div>
                  );
                ...
                servicesPagination.paginatedItems.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="col-span-2 text-gray-400 text-sm">
                        {Number(s._count ?? 0).toLocaleString()}
                      </div>
                      <div className="col-span-3 text-emerald-400 font-bold text-sm">
                        {fmtMoney(s._revenue)}
                      </div>
                      <div className="col-span-2 text-gray-500 text-sm">
                        {s._avgTime}

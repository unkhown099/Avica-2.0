    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div className="text-gray-700">
          Total Staff: <b>{staffAccounts.length}</b>
        </div>
        <button
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden">
          <thead className="bg-gray-50">
            <tr>
              {["ID", "Name", "Email", "Phone", "Role", "Branch", "Status"].map(
                (h) => (
                  <th
                    key={h}
...
function CreateStaffModal({ onClose, roles, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <form
        onSubmit={submit}
        className="bg-white p-10 rounded-xl w-full max-w-lg space-y-5 shadow-lg"
      >
        ...
        <div className="flex gap-4">
          <button
            type="button"
            onClick={onClose}
            className="border px-4 py-2 rounded-lg w-full hover:bg-gray-100 transition"
          >
            Cancel
          </button>
          <button

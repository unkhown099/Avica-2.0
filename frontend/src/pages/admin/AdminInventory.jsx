    ...
    >
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-red-950/50 -m-8 p-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-300">Track and manage parts and supplies inventory</p>
        </div>

        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {stats.map((stat, index) => (
            <div key={index} className={`${stat.cardBg} rounded-xl p-6 shadow-lg border-2 ${stat.borderColor}`}>
        ...
        <div className="bg-gradient-to-br from-red-50 to-orange-50 border-2 border-red-200 rounded-xl p-6 mb-8 shadow-lg">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto text-sm text-gray-600">
              {filteredLowStock.length} {filteredLowStock.length === 1 ? 'item' : 'items'}
            </span>
          </div>
          ...
          <div className="space-y-3">
            {filteredLowStock.map((item, index) => (
              <div key={index} className="bg-white rounded-lg p-4 flex items-center justify-between shadow-sm">
                ...
                <button className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors duration-200 shadow-sm">
                  Reorder
                </button>
              </div>
            ))}
          ...
          {filteredLowStock.length === 0 && (
            <div className="bg-white rounded-lg p-8 text-center">
              ...
              <p className="text-gray-600">No low stock items in selected branch</p>
            </div>
          )}
        </div>

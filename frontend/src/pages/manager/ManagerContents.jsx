...
const mergeScheduleConfig = (raw) => {
  ...
  const mergedDays = DAYS.reduce((acc, day) => {
    acc[day] = { ...base.days[day], ...(raw?.days?.[day] ?? {}) };
    return acc;
  }, {});
  return {
    ...base,
...
function ManagerContents() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setScheduleConfig(
          mergeScheduleConfig(scheduleJson?.config ?? scheduleJson),
        );
      } catch (err) {
  ...
  const sectionMeta = {
    schedule: {
      ...
      subtitle:
        "Define working days, hours, break windows, slot duration, and booking limits.",
    },
    exceptions: {
    ...
    services: {
      ...
      subtitle:
        "Activate, deactivate, and review branch services from this panel.",
    },
  };
  ...
  return (
    <ManagerLayout title="" subtitle="">
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-red-950/30 -m-4 sm:-m-8 p-4 sm:p-6 lg:p-8">
        <div className="mb-4">
        ...
        {activeSection === "exceptions" && (
          <div className="mb-6 flex justify-start sm:justify-end">
            <button
              ...
              className="w-full sm:w-auto px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold transition-colors"
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
              {DAYS.map((day) => {
                ...
                return (
                  <div
                    key={day}
                    className="bg-gray-950/70 border border-white/5 rounded-xl p-4"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <label className="flex items-center gap-2 text-white font-semibold text-sm sm:text-base">
                        <input
                          type="checkbox"
                          className="h-4 w-4 accent-red-500"
                          checked={cfg.enabled}
                      ...
                      <span
                        className={`text-xs px-2 py-1 rounded-full border ${cfg.enabled ? "bg-emerald-500/20 text-emerald-300 border-emerald-500/30" : "bg-gray-700/50 text-gray-400 border-gray-600"}`}
                      >
                    ...
                    <label className="flex items-center gap-2 text-xs text-gray-300 mb-2">
                      <input
                        ...
                        disabled={!cfg.enabled}
                        onChange={(e) =>
                          updateDay(day, { hasBreak: e.target.checked })
                        }
                      />
                      Enable break time
                    </label>
        ...
        {activeSection === "exceptions" && (
          <div className="space-y-3">
            {/* Exception form — stack on mobile */}
            ...
            <div className="flex justify-end mb-4">
              <button
                ...
                className="w-full sm:w-auto px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-semibold"
              >
                Add Override
              </button>
            </div>
            {scheduleConfig.exceptions.length === 0 ? (
              ...
              <div className="space-y-2">
                {scheduleConfig.exceptions.map((item) => (
                  <div
                    key={item.id}
                    className="bg-gray-950/70 border border-white/5 rounded-xl p-3 flex items-start sm:items-center justify-between gap-3"
                  >
                    <div>
                    ...
                    <button
                      ...
                      className="shrink-0 text-xs px-3 py-1.5 rounded-lg bg-red-500/20 text-red-300 border border-red-500/30 hover:bg-red-500/30"
                    >
                      Remove
                    </button>
                  </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-4">
              <div className="grid grid-cols-2 gap-2 w-full sm:w-auto sm:min-w-[360px]">
                <input
                  ...
                  className="bg-gray-900 border border-white/10 text-white rounded-lg px-3 py-2 text-sm"
                />
                <select
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-gray-950/70 border border-white/5 rounded-xl p-3">
                ...
                <div className="text-white text-xl sm:text-2xl font-black mt-1">
                  {services.length}
                </div>
              </div>
              <div className="bg-gray-950/70 border border-white/5 rounded-xl p-3">
              ...
              <div className="bg-gray-950/70 border border-white/5 rounded-xl p-3">
                <div className="text-gray-500 text-xs">In List</div>
                <div className="text-red-300 text-xl sm:text-2xl font-black mt-1">
                  {filteredServices.length}
                </div>
              </div>
            </div>

            {/* Services table — scrollable wrapper so it doesn't overflow on mobile */}
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-white text-sm font-semibold">
                          {service.price_display ||
                            (service.price
                              ? `P${Number(service.price).toLocaleString()}`
                              : "P0")}
                        </td>

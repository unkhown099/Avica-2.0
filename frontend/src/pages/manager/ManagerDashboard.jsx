...
export default function ManagerDashboard() {
  ...
  const transactionsPagination = usePagination({
    items: overviewTransactions,
    pageSize: 10,
    resetDeps: [overviewTransactions.length],
  });
  ...
  const customerBarData = {
    ...
    datasets: [
      {
        label: "Spend (₱)",
        data: customerSpendRows.slice(0, 6).map((c) => Number(c.total_spent ?? 0)),
    ...
    datasets: [
      {
        ...
        data: resolvedTrend.map((t) => Number(t.revenue ?? 0)),
  ...
  const lineChartData = {
    ...
    datasets: [
      ...
      {
        label: "Services",
        data: resolvedTrend.map((t) => Number(t.services ?? 0)),
        borderColor: "#a855f7",
  ...
  const managerKpiCards = useMemo(() => {
      ...
      return managerStatCards.slice(0, 4).map((card) => {
        ...
        if (title.includes("revenue") && valueNumber <= 0 && totalRevenue > 0) {
          return {
            ...
            sub: "From paid branch queue history",
          };
        }
        return card;
      });
    ...
    const uniqueCustomers = new Set(
      appointments.map((a) => String(a.customer_name ?? "").trim()).filter(Boolean),
    ).size;
    const satisfactionValue = stats.find((s) =>
      String(s.title ?? "").toLowerCase().includes("satisfaction"),
    )?.value;

    const fallback = [
      {
        title: "Branch Revenue",
        value: `₱${Number(revenueTotal).toLocaleString()}`,
        sub: "Branch-limited total",
        icon: (
      ...
      },
      {
        ...
        value: Number(doneCount).toLocaleString(),
        sub: "Done appointments",
        icon: (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      },
      {
        ...
        value: Number(uniqueCustomers).toLocaleString(),
        sub: "From branch bookings",
        icon: (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      },
      {
        ...
        value: satisfactionValue ?? "0%",
        sub: "Current branch score",
        icon: (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const handleExportCSV = () => {
    if (activeView === "overview" || activeView === "revenue") {
      ...
      exportToCSV(
        customers.map((c) => [`${c.first_name ?? ""} ${c.last_name ?? ""}`.trim(), c.email ?? "—", c.contact_number ?? "—"]),
        ["Name", "Email", "Contact"],
        "manager_customers.csv",
      );
    } else if (activeView === "appointment") {
      exportToCSV(
        appointments.map((a) => [a.date ?? "—", a.time ?? "—", a.customer_name ?? "—", a.service ?? "—", a.status ?? "—"]),
        ["Date", "Time", "Customer", "Service", "Status"],
        "manager_appointments.csv",
      );
    } else if (activeView === "inventory") {
      exportToCSV(
        inventory.map((i) => [i.name ?? "—", i.category ?? "—", i.quantity ?? 0, i.unit ?? "—", i.status ?? "—"]),
        ["Item", "Category", "Quantity", "Unit", "Status"],
        "manager_inventory.csv",
      );
    } else if (activeView === "services") {
      exportToCSV(
        services.map((s) => [s.service ?? "—", s.count ?? 0, s.revenue ?? 0]),
  ...
  const handlePrintDashboard = async () => {
    ...
    printWindow.document.write(`
      <html><head><title>Manager Dashboard - ${viewLabel}</title></head>
    ...
    <ManagerLayout title="" subtitle="">
      <style>{`
        @media print {
          ...
          #manager-dashboard-print [class*="bg-"] {
            background: transparent !important;
          }
          #manager-dashboard-print [class*="border-"] {
            border-color: #d1d5db !important;
          }
          #manager-dashboard-print canvas {
            max-width: 100% !important;
            height: auto !important;
          }
        }
      `}</style>
      <div id="manager-dashboard-print" className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-red-950/30 -m-8 p-8 print:bg-white print:p-4">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-white tracking-tight">Manager Dashboard</h1>
          <p className="text-gray-400 mt-1">Welcome back — {branchName}</p>
        </div>
          ...
          <>
              ...
              <section id="manager-overview" className="scroll-mt-24 mb-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
                  {loading
                    ...
                    : managerKpiCards.map((card, i) => <StatCard key={i} {...card} />)}
                </div>
                ...
                <div className="bg-gray-900/60 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-sm">
                  ...
                  {loading ? (
                    ...
                    transactionsPagination.paginatedItems.map((row, i) => {
                      ...
                      return (
                        <div key={i} className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 hover:bg-white/[0.02] items-center">
            ...
            {activeView === "revenue" && (
              <section id="manager-revenue" className="scroll-mt-24 mb-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                  {[
                    ...
                    },
                    {
                      ...
                      value: Number(bookingsToday).toLocaleString(),
                    ...
                    },
                    {
                      ...
                      accentText: "text-emerald-400",
                      border: "border-emerald-500/20",
                      sub: `Paid rate: ${paymentRate.toFixed(1)}%`,
                      icon: (
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                  <div className="xl:col-span-2 bg-gray-900/60 border border-white/5 rounded-2xl p-6 backdrop-blur-sm">
                    <h3 className="text-lg font-black text-white mb-1">Monthly Revenue Trend</h3>
                    <p className="text-gray-500 text-sm mb-6">Revenue movement across recent months</p>
                    <div className="h-72">
                      <Bar data={revenueMonthlyBarData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: "#9ca3af" } } }, scales: { x: { ticks: { color: "#6b7280" }, grid: { color: "rgba(255,255,255,0.04)" } }, y: { ticks: { color: "#6b7280" }, grid: { color: "rgba(255,255,255,0.04)" } } } }} />
                    </div>
                  </div>
                  <div className="bg-gray-900/60 border border-white/5 rounded-2xl p-6 backdrop-blur-sm">
                    <h3 className="text-lg font-black text-white mb-1">Revenue Contribution</h3>
                    <p className="text-gray-500 text-sm mb-6">Share of each month in total revenue</p>
                    <div className="h-72">
              ...
              <section id="manager-customers" className="scroll-mt-24 mb-10">
                  ...
                  {[
                    {
                      ...
                      value: Number(customers.length).toLocaleString(),
                      accentBg: "bg-purple-500/10",
                      accentText: "text-purple-400",
                      border: "border-purple-500/20",
                      icon: (
                    ...
                    },
                    {
                      ...
                      value: Number(newCustomers30d).toLocaleString(),
                      accentBg: "bg-blue-500/10",
                      accentText: "text-blue-400",
                      border: "border-blue-500/20",
                      icon: (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
                  <div className="bg-gray-900/60 border border-white/5 rounded-2xl p-6 backdrop-blur-sm">
                    ...
                    <div className="h-56">
                      <Bar data={customerBarData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: "#9ca3af" } } }, scales: { x: { ticks: { color: "#6b7280" }, grid: { color: "rgba(255,255,255,0.04)" } }, y: { ticks: { color: "#6b7280" }, grid: { color: "rgba(255,255,255,0.04)" } } } }} />
                    </div>
                  </div>
                  <div className="bg-gray-900/60 border border-white/5 rounded-2xl p-6 backdrop-blur-sm">
                        ...
                        <div key={t.tier} className="flex items-center gap-4">
                          ...
                          <div className="flex-1 h-2 bg-gray-800 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${customerSpendRows.length ? (t.count / customerSpendRows.length) * 100 : 0}%`,
                ...
                <div className="bg-gray-900/60 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-sm">
                  ...
                  {customersPagination.paginatedItems.map((c, i) => {
                    ...
                    return (
                      <div key={i} className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 hover:bg-white/[0.02] transition-colors items-center">
                        ...
                        <div className="col-span-1">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${SEGMENT_STYLE[segment]}`}>
                            {segment}
                          </span>
                        </div>
                      </div>
                  ...
                  <Pagination current={customersPagination.currentPage} total={customersPagination.totalPages} onChange={customersPagination.setCurrentPage} className="px-6 py-4" />
                </div>
              </section>
            )}
            ...
            {activeView === "appointment" && (
              <section id="manager-appointment" className="scroll-mt-24 mb-10">
                ...
                <div className="bg-gray-900/60 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-sm">
                  <div className="px-6 py-4 border-b border-white/5">
                    <h3 className="text-lg font-black text-white">Appointment Queue</h3>
                  ...
                  {appointmentsPagination.paginatedItems.map((a) => {
                    ...
                    return (
                      <div key={a.id} className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 hover:bg-white/[0.02] transition-colors items-center">
                        ...
                        <div className="col-span-1">
                          <span className={`px-2 py-1 rounded-full text-xs border ${STATUS_STYLE[statusKey] ?? "bg-gray-500/20 text-gray-300 border-gray-500/30"}`}>
                            {STATUS_LABEL[statusKey] ?? (a.status || "—")}
                          </span>
                        </div>
                      </div>
                    );
                  })}
                  <Pagination current={appointmentsPagination.currentPage} total={appointmentsPagination.totalPages} onChange={appointmentsPagination.setCurrentPage} className="px-6 py-4" />
                </div>
              </section>
            )}
              ...
              <section id="manager-inventory" className="scroll-mt-24 mb-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                  {[
                    ...
                    },
                    {
                      ...
                      value: String(inventoryCounts.low),
                    ...
                    },
                    {
                      ...
                      value: String(inventoryCounts.critical + inventoryCounts.out),
                ...
                <div className="bg-gray-900/60 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-sm">
                  ...
                  {inventoryPagination.paginatedItems.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 hover:bg-white/[0.02] transition-colors items-center">
                        ...
                        <div className="col-span-2 text-gray-300 text-sm font-bold">
                          {Number(item.quantity ?? 0)} <span className="text-gray-600 font-normal">{item.unit ?? ""}</span>
                        </div>
                        ...
                        <div className="col-span-2">
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${INV_STYLE[invStatus]}`}>
                            {invStatus === "ok" ? "Available" : invStatus === "low" ? "Running Low" : invStatus === "critical" ? "Reorder Now" : "Out of Stock"}
                          </span>
                        </div>
                      </div>
                    );
                  })}
                  <Pagination current={inventoryPagination.currentPage} total={inventoryPagination.totalPages} onChange={inventoryPagination.setCurrentPage} className="px-6 py-4" />
                </div>
              </section>
            )}
                  ...
                  {[
                    {
                      ...
                      value: String(services.reduce((a, s) => a + Number(s.count ?? 0), 0)),
                      accentBg: "bg-blue-500/10",
                      accentText: "text-blue-400",
                      border: "border-blue-500/20",
                      icon: (
                    ...
                    },
                    {
                      ...
                      value: `₱${services.reduce((a, s) => a + Number(s.revenue ?? 0), 0).toLocaleString()}`,
                      accentBg: "bg-emerald-500/10",
                      accentText: "text-emerald-400",
                      border: "border-emerald-500/20",
                      icon: (
                ...
                <div className="bg-gray-900/60 border border-white/5 rounded-2xl overflow-hidden backdrop-blur-sm">
                  ...
                  {services.length === 0 ? (
                    ...
                    servicesPagination.paginatedItems.map((s, idx) => {
                      ...
                      return (
                        <div key={`${s.service}-${idx}`} className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/5 hover:bg-white/[0.02] transition-colors items-center">
